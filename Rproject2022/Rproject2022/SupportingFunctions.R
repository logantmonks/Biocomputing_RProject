...
# Function that converts all files into .csv
...
fileConvert<-function(filename){ # filename acts as placeholder for file of interest
  for(i in 1:length(filename)){
    ...
    # setwd to desired location of converted .csv files
    write.table(DATA,file=paste0("/Users/matha/Desktop/RProject/Test",sub(".txt","",filename[i]),".csv"),row.names=F,quote=F,sep=",") 
    ###The pathway leads to a folder names "Test" that had been previously created
  }
}
#fileConvert(countryYfileConvert) # specify here which filename you wish to use with the function
...
#merge(rproject, na_handling="warn of NA presence") ## runs the second function with warning for NA presence
###End function 2
...
data_summary<-function(file) {
  ...
  Days<-c(summary_table[,14]) # pulls DoY data from dayofyear column
